...
export default class Client extends React.Component {
    ...
    updateClient = (slug) => {
        ...
        document.title = `${client.name} | Gavin Thomas`
    }
    ...
    renderClient = () => {
        ...
        return ( <
            ...
            <
            h3 > Details < /h3> <
            ul > { duration && < Detail title = "Duration"
                value = { duration }
                text / > } { location && < Detail title = "Location"
                value = { location }
                text / > } { scope && < Detail title = "Scope"
                value = { scope }
                text / > } { stack && < Detail title = "Stack"
                value = { stack }
                text / > } { website && < Detail title = "Website"
                value = { website }
                link / > } { github && < Detail title = "Github"
                value = { github }
                link / > } { appstore && < Detail title = "App Store"
                value = { appstore }
                btn / > } {
                playstore &&
                    <
                    Detail title = "Play Store"
                value = { playstore }
                btn / >
            } <
            /ul> <
            ...
            /div> <
            PrevNext allClients = { allClients }
            clientId = { this.state.order }
            /> <
            /div>
        )
    }
